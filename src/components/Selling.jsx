...
const Selling = () => {
    ...
    return (
        <div className='flex flex-row my-20 mx-10' id='products'>
            <div className="w-[26%] mr-20">
                ...
                <button className='primary my-4 w-[160px] h-[40px] rounded-md font-medium flex  justify-center items-center shadow hover:scale-110 transition'>{btnText}<RiArrowRightLine className='ml-2 text-[18px] hover:translate-x-1 transition ease-in-out' /></button> 
            </div>

                {brand.map((brand, index) => {
                    const { name, price , image } = brand;
                    return (
                        <div className="flex mx-12" key={index}>
                            <div className="">
